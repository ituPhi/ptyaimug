
## Reunión y presentación mensual de Febrero

Grupo de Usuarios de Middleware de IA (AIMUG)
Esta reunión se realizará en español.

- Fecha: Miércoles, 25 de febrero de 2026
- Hora: 18:00 – 20:00
- Oficina. Green Valley Panamá SA Calle 54 Este, Casa 9 (Marbella) Ciudad de Panamá.
- Tendremos capacidad limitada, así que por favor confirma tu asistencia y llega a tiempo.
- Ubicación: [Calle 54 Este, Casa 9 (Marbella) Ciudad de Panamá.](https://maps.app.goo.gl/LNvPfDV43h84YjWM8)

### Presentaciones (Lightning Talks)
- Iván Darío Gómez Rincón (Director of AI, Liberty Latin America): "Building AI Sales Bots with a Scalable Amazon Bedrock Agent Core Framework"
- Carlos Luis Salazar: "IA Multi-Modelo con un Clic: Despliega GPT-4o, Embeddings y Azure AI Search sin saber de Infraestructura"
- Livan Gonzalez: "Kody - REPL with AI using Le Mistral"

### Agenda
- 18:00 – 18:20 Networking
- 18:20 – 18:35 Bienvenida & Presentación
- 18:35 - 18:50 Introducción al AI Middleware
- 18:50 - 19:00 Noticias recientes del AI Middleware
- 19:00 – 19:45 Lightning Talks (3 espacios)
- 19:45 - 20:00 Networking final + transición a off-site
- 20:00 - ??? Ubicación de discusión fuera del sitio Rana Dorada Marbella

***
### Acerca de AIMUG
El Grupo de Usuarios de Middleware de IA cree en el "Aprendizaje Abierto". Experimentamos con LangChain, LangGraph, LangSmith, compartiendo demostraciones, dificultades y logros para que todos mejoren.

### Acerca del Grupo de Reunión AIMUG de Panamá
El Grupo de Reunión AIMUG de Panamá está afiliado al Grupo de Reunión Austin Langchain en Austin, Texas, EE. UU., posiblemente la comunidad Langchain más grande y mejor organizada del mundo.
***

## February Monthly Meeting and Presentation
AI Middleware User Group (AIMUG)
This meeting will be held in Spanish.

- Date: Wednesday, February 25, 2026
- Time: 6:00 PM – 8:00 PM
- Office: Green Valley Panamá SA, Calle 54 Este, Casa 9 (Marbella), Panama City
- Space is limited, so please RSVP and arrive on time.
- Location: [Calle 54 Este, Casa 9 (Marbella) Ciudad de Panamá.](https://maps.app.goo.gl/LNvPfDV43h84YjWM8)

### Lightning Talks
- Iván Darío Gómez Rincón (Director of AI, Liberty Latin America): "Building AI Sales Bots with a Scalable Amazon Bedrock Agent Core Framework"
- Carlos Luis Salazar: "IA Multi-Modelo con un Clic: Despliega GPT-4o, Embeddings y Azure AI Search sin saber de Infraestructura"
- Livan Gonzalez: "Kody - REPL with AI using Le Mistral"

### Agenda
- 6:00 PM – 6:20 PM Networking
- 6:20 PM – 6:35 PM Welcome and Group Introductions
- 6:35 PM – 6:50 PM Introduction to AI Middleware
- 6:50 PM – 7:00 PM Latest News on AI Middleware
- 7:00 PM – 7:45 PM Lightning Talks
- 7:45 PM – 8:00 PM Networking and Transition to Working Outside the Office
- 8:00 PM – ??? Off-site Discussion Location La Rana Dorada Marbella

### About AIMUG
The AI Middleware User Group believes in "Open Learning." We experiment with LangChain, LangGraph, LangSmith, and the broader AI stack, sharing demos, challenges, and successes to help everyone improve.
### About the AIMUG Panama Meeting Group
The AIMUG Panama Meeting Group is affiliated with the Austin Langchain Meeting Group in Austin, Texas, USA, arguably the largest and best-organized Langchain community in the world.
